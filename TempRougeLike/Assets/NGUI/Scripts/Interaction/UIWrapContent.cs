...
public class UIWrapContent : MonoBehaviour
{
	...
	public virtual void SortBasedOnScrollMovement ()
	{
        ...
        var temp = mGrid == null ? mTrans : mGrid.transform; // changed by KPH
        for (int i = 0; i < temp.childCount; ++i)
		{
			Transform t = temp.GetChild(i);
	...
	{
        if (mGrid) // changed by KPH
        {
            for (int i = 0, imax = mChildren.Count; i < imax; ++i)
            {
                Transform t = mChildren[i];
                int x = i / mGrid.maxPerLine;
                int y = i % mGrid.maxPerLine;
                t.localPosition = mHorizontal ? new Vector3(x * itemSize,  y * mGrid.cellHeight, 0f) : 
                                                new Vector3(y * mGrid.cellHeight, -x * itemSize, 0f);
                UpdateItem(t, i);
            }
            return;
        } // chaged end
	...
	protected virtual void UpdateItem (Transform item, int index)
	{
		if (onInitializeItem != null)
		{
            ...
            if (mGrid)
            {
                var temp = index % mGrid.maxPerLine + mGrid.maxPerLine * Mathf.Abs(realIndex);
                onInitializeItem(item.gameObject,  temp, realIndex);
            }
            else
