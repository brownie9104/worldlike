		...
		if (del.target != null && del.target.gameObject != null)
		{
			...
			if (ps != null)
			{
				for (int i = 0; i < ps.Length; ++i)
				{
                    ...
                    string name = "";
                    if (param.expectedType == typeof(int))
                    {
                        name = "Int";
                    }
                    ...
                    else if (param.expectedType == typeof(string))
                    {
                        name = "String";
                    }
                    ...
                    if (isButtonOn == true)
                    {
                        if (GUILayout.Button(name + (char)0x200a, GUILayout.Width(60), GUILayout.Height(15)))
                        {
                            param.IsCustomValue = !param.IsCustomValue;
                        }
                    }
                    else
                    {
                        GUILayout.Label("   " + name, GUILayout.Width(60), GUILayout.Height(15));
                    }
                    GUILayout.Space(10f);
                    if (param.IsCustomValue == true)
                    {
                        if (param.expectedType == typeof(int))
                        {
                            param.CustomValueInt = EditorGUILayout.IntField("", param.CustomValueInt);
                        }
                        else if (param.expectedType == typeof(float))
                        {
                            param.CustomValueFloat = EditorGUILayout.FloatField("", param.CustomValueFloat);
                        }
                        else if (param.expectedType == typeof(string))
                        {
                            param.CustomValueString = EditorGUILayout.TextField("", param.CustomValueString);
                        }
                        else if (param.expectedType == typeof(bool))
                        {
                            param.CustomValueBool = EditorGUILayout.Toggle("", param.CustomValueBool);
                        }
                        EditorGUIUtility.labelWidth = 90f;
                        GUILayout.EndHorizontal();
                        continue;
                    }
                    Object obj = EditorGUILayout.ObjectField(param.obj, typeof(Object), true);
                    EditorGUIUtility.labelWidth = 90f;
                    GUILayout.EndHorizontal();
